...
import (
	...
	genclient "github.com/Gys/goa/goagen/gen_client"
...
var _ = Describe("Generate", func() {
	const testgenPackagePath = "github.com/Gys/goa/goagen/gen_client/test_"
...
`))
	...
	Context("with an action with a user type payload", func() {
		...
		It("generates the user type imports", func() {
			...
			content, err := ioutil.ReadFile(filepath.Join(outDir, "client", "user_types.go"))
			Ω(err).ShouldNot(HaveOccurred())
			Ω(content).Should(ContainSubstring("uuid \"github.com/Gys/goa/uuid\""))
		})
	})
